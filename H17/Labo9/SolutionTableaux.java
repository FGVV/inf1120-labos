...
public class SolutionTableaux {
    ...
    public static boolean superieurAzero(int[] tab){
        ...
        if (tab != null){
            ...
            // si le tableau est vide alors z�ro it�ration pour la boucle while et 'supAzero' reste �gale � true
    ...
    public static int maximum (int[] tab){
        ...
        if (tab != null && tab.length != 0){
            ...
            // Initialisation de max au premier �l�ment du tableau
            max = tab[0];
            for(int i = 0; i < tab.length; i++){
                if (tab[i] > max){
                    max = tab[i];
    ...
    public static int[] epure (int[] tab){
        ...
        if (tab != null){
            // compter le nombre de valeurs diff�rentes de zero = taille du nouveau tableau
    ...
    public static int[] suite (int a, int b){
        ...
        int tabSuite[] = {};
        int tailleTabSuite; // taille du tableau tabSuite qui d�pend de la valeur de a et b

        if (a <= b ){
            // calcul de la taille du tableau tabSuite 
            tailleTabSuite =  b - a + 1;
            // r�servation de l'espace pour le tableau tabSuite
    ...
    public static String[] prefixe (String ch){
        ...
        if (ch != null ){
            ...
            // reservation de l'espace pour le tableau tabPrefixe
            tabPrefixe = new String [tailleTabPrefixe];
            tabPrefixe[0] = "";

            for(int i = 1; i <= ch.length(); i++){
                tabPrefixe[i] = tabPrefixe[i-1] + ch.charAt(i-1); // vous pouvez aussi utiliser la m�thode substring() pour extraire les prefixes
